...
class StopAndWaitSegment:
    ...
    def serialize(self):
        ...
        payload_len_bytes = len(self.payload).to_bytes(
            2, byteorder="big")  # 2 bytes
    ...
    def deserialize(data):
        ...
        crc_received = int.from_bytes(
            data[3 + payload_len:3 + payload_len + 4], byteorder="big")
...
class InitSegment:
    ...
    def serialize(self):
        ...
        packet_to_crc = header_bytes + file_name_len_bytes + \
            self.file_name + file_path_len_bytes + self.file_path
    ...
    def deserialize(data):
        ...
        crc_received = int.from_bytes(
            data[3 + file_name_len + file_path_len:], byteorder="big")

        # Check the CRC
        crc_calculated = zlib.crc32(
            data[:3 + file_name_len + file_path_len]) & 0xFFFFFFFF
        if crc_calculated != crc_received:
            raise ValueError("CRC mismatch")
